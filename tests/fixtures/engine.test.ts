...
function take(state: MatchState, format: MatchFormat, side: "user" | "opponent", result: "scored" | "saved" | "missed"): MatchState {
  return applyPenaltyAttempt(state, format, { side, takerId: `${side}-${state.events.length}`, result }, null);
}

describe("종료 상태기", () => {
  it("정규시간 리드를 regulation 결과로 끝낸다", () => {
    const result = stepPhase(state({ userGoals: 1 }), fullExtraTime, null).state;
  ...
  it("연장과 승부차기 없는 정규시간 동점을 무승부로 끝낸다", () => {
    ...
    const result = stepPhase(state(), format, null).state;
  ...
  it("풀 연장전 득점은 종료 시점까지 진행한 뒤 연장 결과로 확정한다", () => {
    const entered = stepPhase(state(), fullExtraTime, null).state;
    const scored = applyGoal(entered, fullExtraTime, { side: "user", scorerId: "scorer", kind: "openPlay" }, null);
    const result = stepPhase({ ...scored, clock: { ...scored.clock, minute: 30, absoluteMinute: 120 } }, fullExtraTime, null).state;
  ...
  it("골든골 연장전 득점은 남은 시간을 소진하지 않고 즉시 끝낸다", () => {
    ...
    const entered = stepPhase(state(), format, null).state;
  ...
  it("연장 동점 뒤 정규 다섯 라운드 4대 3 승부차기를 확정한다", () => {
    const entered = stepPhase({ ...state(), clock: { phase: "extraTime", minute: 30, absoluteMinute: 120, shootoutRound: null } }, fullExtraTime, null).state;
    let result = entered;
  ...
  it("남은 킥을 모두 성공해도 뒤집지 못하면 조기 종료한다", () => {
    ...
    let result = stepPhase(state(), format, null).state;
    for (const [side, outcome] of [
      ["user", "scored"], ["opponent", "missed"],
      ["user", "scored"], ["opponent", "missed"],
      ["user", "scored"], ["opponent", "missed"],
  ...
  it("사용자가 진 팀을 지휘해도 관점 상대적으로 패배를 반환한다", () => {
    const result = stepPhase(state({ opponentGoals: 1 }), fullExtraTime, null).state;
