...
export function Dugout({ scenarioId, tokenIndex, initialFormation, initialPlacements, initialDirectives, onConfirm, onClose }: DugoutProps) {
  ...
  return (
    <section className="dugout-overlay" role="dialog" aria-modal="true" aria-label="더그아웃 전술 편집">
      ...
      <div ref={pitchRef} className="dugout-pitch" onPointerMove={drag.onPointerMove} onPointerCancel={drag.onPointerCancel}>
        <PitchLines />
        ...
        {placements.map((placement) => {
          ...
          return <button key={placement.playerId} type="button" className={`player-token ${drag.state.itemId === placement.playerId ? "is-dragging" : ""}`} style={{ ...DRAG_HANDLE_STYLE, top: `${placement.slot.x}%`, left: `${placement.slot.y}%` }} onPointerDown={(event) => drag.onPointerDown(event, placement.playerId)} onPointerUp={finishDrag} onPointerMove={drag.onPointerMove} onPointerCancel={drag.onPointerCancel} aria-label={`${player.label}, ${player.position}${player.confirmed ? "" : ", 재구성"}`}><span>{player.label}</span><small>{player.position}</small></button>;
        })}
      </div>
      ...
      <section className="bench" aria-label="벤치 선수" style={SCROLL_REGION_STYLE}><p>벤치, 교체 카드 {3 - cardsUsed}장</p><div className="bench-scroll">{squad.bench.filter((player) => !placements.some((placement) => placement.playerId === player.id)).map((player) => <button key={player.id} type="button" className="bench-card" style={DRAG_HANDLE_STYLE} onPointerDown={(event) => drag.onPointerDown(event, `bench:${player.id}`)} onPointerUp={finishDrag} onPointerMove={drag.onPointerMove} onPointerCancel={drag.onPointerCancel}>{player.label}<small>{player.position}{player.confirmed ? "" : " 재구성"}</small></button>)}</div></section>
