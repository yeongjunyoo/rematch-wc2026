...
| YouTube 시연영상 | **파일 준비됨, 업로드만 남음** | `packaging/_footage/20260731-170833/REMATCH_시연영상.mp4` (50.4초, 1280x720, 2.7MB) | 유튜브 업로드 뒤 익명 재생 확인 |
...
2. **YouTube 시연영상 업로드**
   영상은 만들어져 있습니다. 파일은 `packaging/_footage/20260731-170833/REMATCH_시연영상.mp4`이고,
   로그인과 2단계 인증이 필요한 업로드만 남았습니다. 요강이 요구하는 네 가지가 모두 들어 있습니다.
   - 시작 화면 (다섯 미션 목록)
   - 선수 배치와 전술 설정 (더그아웃, 포메이션 프리셋)
   - 핵심 상호작용 (손흥민 교체와 개입 확정, 경기 재생과 피드)
   - 결과 화면 (결과 리포트와 실제 역사 대조)
   업로드 뒤 로그아웃 상태에서 재생되는지 확인합니다.
   제목 후보: `REMATCH — 손흥민이 벤치에 있던 그 밤을, 당신이 다시 지휘한다`
